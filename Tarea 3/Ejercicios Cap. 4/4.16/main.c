...
// El programa recibe como datos 24 números reales que representan las temperaturas en el exterior en un período de 24 horas. Calcula el
// promedio del día y las temperaturas máxima y mínima con la hora en la que se registraron.
void Acutem(float);
...
int HMAX;
int HMIN;
// Variables globales. ACT se utiliza para acumular las temperaturas, por esa razón se inicializa en cero. MAX se utiliza para calcular la máxima;
// Se inicializa en –50 para que el primer valor que se ingrese modifique su contenido. MIN se usa para calcular la mínima;
// se inicializa con un valor muy alto para que el primer valor ingresado modifique su contenido.
